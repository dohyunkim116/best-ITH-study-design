...
Assume $K_{max}$ is the integer that maximizes $\xi(K)$. Then, the optimal design is to choose $N=M/K_{max}$ patients and $K_n=K_max$ for each $n = 1,...,N$. The optimal value is given by
...
The function `recursive_search()` defined in $\texttt{recursive_search.cpp}$ computes the optimal design with following inputs:
