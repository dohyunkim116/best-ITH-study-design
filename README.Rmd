...
This document partly reproduces numerical studies presented in the best ITH (intratumor heterogeneity) association study [paper](). Specifically, we will

1.  simulate (binary) multiregion genomic data,
2.  estimate parameters that will be used to find optimal designs, and
3.  find optimal designs for pre-collection and post-collection scenario for given estimated parameters.

All of the functions appearing in this document is in $\texttt{functions.R}$.
...
Given the budget to profile $M$ tumor samples, we want to find $(N,K_1,...,K_n)$ (optimal design) that maximize $\varphi$ under the constraints $K_n \geq 2$ and $\sum_{i=1}^N K_i \leq M.$
...
Assume $K_{max}$ is the integer that maximizes $\xi(K)$. Then, the optimal design is to choose $N=M/K_max$ patients and $K_n=K_max$ for each $n = 1,...,N$. The optimal value is given by
$$
...
In the following example, we set $M = 100$ and search $K_{max} \in \{2,3,...,10\}$ that maximizes $\varphi$ for the following parameters producing different $K_{max}$:

- $(\hat{\tau}^2,\hat{\sigma}^2,\hat{\rho}) = (4.718, 5.831, 1.463)$ (2nd row)
- $(\hat{\tau}^2,\hat{\sigma}^2,\hat{\rho}) = (3.123, 6.527, 0.877)$ (8th row)
- $(\hat{\tau}^2,\hat{\sigma}^2,\hat{\rho}) = (2.009, 6.800, 0.147)$ (10th row)

The function `phi1()` takes $\sigma^2$, $\rho$ and $\tau^2$, number of samples $K$ and a total budget $M$ as inputs and outputs $\varphi$.
...
where $f(a,\sigma^2,\rho)$ is identical to the pre-collection scenario setting. The details of constraints can be found in the [paper](link).
...
The function `recursive_search()` (in $\texttt{recursive_search.cpp}$) computes the optimal design with following inputs:
